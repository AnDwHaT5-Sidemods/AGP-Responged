	...
	public <E extends List> void fillContents(int rows, E gyms)
	{
		for (int i = 0; i < gyms.size() && i < 36; i++)
		{
			...
			lore.appendTag(new NBTTagString(TextFormatting.GRAY + "Status: " + (
					gym.Status == 0 ? TextFormatting.GREEN + "Open" :
					gym.Status == 1 ? TextFormatting.RED + "Closed" :
			...
			lore.appendTag(new NBTTagString(TextFormatting.GRAY + "Leaders:"));

			PlayerList p = FMLCommonHandler.instance().getMinecraftServerInstance().getPlayerList();
			int npcs = 0;
			List<String> onlineLeaders = new ArrayList<>();
			List<String> offlineLeaders = new ArrayList<>();
			for (String leader : gym.Leaders)
			{
				...
				{
					boolean online = Arrays.stream(p.getOnlinePlayerNames()).anyMatch(pl -> pl.equalsIgnoreCase(leader));
					if (online)
					{
			...
			for (String leader : onlineLeaders)
			{
			...
			for (String leader : offlineLeaders)
			{
				lore.appendTag(new NBTTagString("  " + TextFormatting.YELLOW + leader + TextFormatting.GRAY +
						" (" +
